...
int main() {
    ...
    std::uniform_real_distribution<double> interval(P_MIN, P_MAX);
    ...
    for (int i = 0; i < m; ++i) {
        if (GEN_RAND_PTS) {
            tb[i][0] = interval(_random);
            tb[i][1] = interval(_random);
        } else {
            cin >> tb[i][0] >> tb[i][1];
        }
    }
    ...
    for (int i = b.rows() - 1; i >= 0; --i) {
        par.append(std::to_string(b[i][0]));
        par.append("*x**");
        par.append(std::to_string(i));
    ...
    fprintf(pipe, "plot %s, '-' " \
                  "using 1:2 with points title \"random points\"\n", par.c_str());

//    printf("plot %s, '-' using 1:2 with points\n", par.c_str());
    for (int i = 0; i < m; ++i) {
        if (GEN_RAND_PTS)
            printf("%lf\t%lf\n", tb[i][0], tb[i][1]);
        fprintf(pipe, "%lf\t%lf\n", tb[i][0], tb[i][1]);
